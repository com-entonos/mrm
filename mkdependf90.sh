...
find_mod_inc() {
	...
	for j in $(grep -i "^ *use "  $1 | awk '{print tolower($0)}' | sed "s/,/ /g; s/::/ /; s/ non_intrinsic / /; s/ intrinsic / /" | \
		...
		moddef=""
       	 	for i in $(find . -name "*.f90" | grep -v $1 | sed "s/^\.\///" | xargs grep -i "^ *module *${j}" | \
			...
			(( $(grep -i "^ *module *${j}" $i | awk '{print tolower($2)}' | \
				...
				moddef="1" && [ ! -n "${onlymod}" ] && deps+=" ${blddir}${i/.f90/.o}"  #add correspond source object to dependencies
#				moddef="1" && [[ "$i" != *"grafix.f90" ]] && \
#				[ ! -n "${onlymod}" ] && deps+=" ${blddir}${i/.f90/.o}"  #add correspond source object to dependencies
        	done
	...
	for i in $(grep -i "^ *include " $1 | sed "s/\"/ /g; s/\'/ /g"| awk '{print $2}'| sort -u);do
		j=$(find . -name $i) && (( $(find . -name $i | wc -l) > 0 )) && deps+=" ${j}$(find_mod_inc ${j})" #recursion!
	done
	echo "${deps}"
}

[ -n "$3" ] && onlymod="1"
blddir="$2" && sfile="$1" && mkline="${blddir}${sfile/.f90/.o} : ${sfile}$(find_mod_inc ${sfile})" #object file : source file
